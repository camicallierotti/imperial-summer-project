...
xgboost_model <- xgboost(data = as.matrix(X_train), # xgboost library requires data as matrix
                 ...
                 nrounds = 2, # number of times we want xgboost to run
                 eta = 1,
                 verbose = 1
                 )
...
pred <- predict(xgboost_model, as.matrix(X_test)) # perform the prediction of risk scores
print(length(pred)) # size of the prediction vector (confirm 56 same as test set)
print(head(pred)) # display first predictions: probabilities that a datum will be classified as 1 (event happening)
prediction <- as.numeric(pred > 0.5) # convert to probability of dead or alive

### c-index 

y_for_cindex <- ifelse(y_test < 0, 0, 1) # convert censored to 0 and death to 1
cindex <- survConcordance(y_for_cindex ~ prediction) # 0.17???
